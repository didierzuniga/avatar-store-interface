...
import { useAvatarsStoreData } from "../../hooks/useAvatarsStoreData";
...
const Avatars = () => {
  ...
  const { avatar, loading } = useAvatarsStoreData({ owner: submitted && validAddress ? address : null });
  ...
  const submit = (event) => {
    ...
    if (address) {
      ...
      setSubmitted(true);
      if (isValid) push(`/avatars?address=${address}`);
    } else {
      push("/avatars");
    }
  };
